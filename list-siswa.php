...
<head>
    ...
    <title>Data Pendaftaran Siswa | SMK Krian 1</title>
    ...
    <style>
        ...
        /* Custom Scrollbar untuk Table Area */
...
<body class="bg-[#F5F3FF] font-sans antialiased text-pastel-dark min-h-screen flex flex-col md:flex-row relative overflow-x-hidden p-0 sm:p-4 md:p-6 gap-0 sm:gap-6">
        ...
        <div>
            ...
            <div class="space-y-2">
                ...
                <a href="index.php" onclick="navigasiDenganAnimasi(event, this.href)" 
                   class="menu-item flex items-center gap-3.5 px-4 py-3.5 rounded-2xl text-slate-600 hover:bg-white hover:text-pastel-purple font-semibold text-sm transition-all group hover:shadow-md hover:shadow-purple-500/5 hover:-translate-y-0.5 no-underline">
                ...
                <!-- Link Data Pendaftar (Active) -->
                <a href="list-siswa.php" class="menu-item flex items-center justify-between px-4 py-3.5 rounded-2xl bg-gradient-to-r from-pastel-purple to-indigo-600 text-white font-bold text-sm shadow-lg shadow-purple-500/30 transition-all hover:scale-[1.02] no-underline">
                    ...
                    <span class="text-[10px] bg-white/20 text-white font-bold px-2.5 py-1 rounded-full backdrop-blur-md">Aktif</span>
                </a>
            </div>
        </div>
    ...
    <main class="flex-1 min-w-0 p-4 sm:p-6 flex flex-col justify-between z-10">
        ...
        <div id="mainContent" class="max-w-5xl mx-auto w-full my-auto animate-pop-in">
            
            <!-- Main Data Table Card -->
            <div class="glass-card rounded-[2.5rem] p-6 sm:p-8 relative overflow-hidden my-4 sm:my-0">
                ...
                <!-- Header Section -->
                <header class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
                    <div>
                        <a href="index.php" onclick="navigasiDenganAnimasi(event, this.href)" 
                           class="inline-flex items-center gap-2 text-xs font-bold text-pastel-purple hover:text-indigo-800 bg-purple-50 hover:bg-purple-100 px-3.5 py-1.5 rounded-full transition-all duration-200 mb-3 border border-purple-100 shadow-sm w-fit group no-underline">
                            <i class="fa-solid fa-arrow-left group-hover:-translate-x-1 transition-transform"></i> Kembali ke Beranda
                        </a>
                        ...
                        <p class="text-slate-500 text-xs sm:text-sm mt-1 font-medium">
                            Daftar calon siswa yang telah terdaftar pada sistem PPDB online.
                        </p>
                    </div>

                    <!-- Tombol Tambah Pendaftaran Baru -->
                    <div>
                        <a href="form-daftar.php" onclick="navigasiDenganAnimasi(event, this.href)" class="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-gradient-to-r from-pastel-purple via-indigo-600 to-pastel-teal text-white font-bold text-xs sm:text-sm shadow-lg shadow-purple-500/25 hover:shadow-xl hover:shadow-purple-500/35 hover:-translate-y-0.5 active:translate-y-0 transition-all duration-300 no-underline">
                ...
                <!-- Table Container -->
                <div class="overflow-x-auto custom-scrollbar rounded-2xl border border-purple-100 bg-white/70 backdrop-blur-md relative z-10 shadow-sm">
                    <table class="w-full text-left border-collapse text-xs sm:text-sm">
                        <thead class="bg-purple-50/80 border-b border-purple-100 text-slate-700 font-extrabold uppercase tracking-wider text-[11px]">
                            <tr>
                                ...
                                <th class="py-4 px-4 text-center">Agama</th>
                                <th class="py-4 px-4">Sekolah Asal</th>
                                <th class="py-4 px-4 text-center">Tindakan</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-purple-50/80">
                            ...
                            if ($total > 0) {
                                while($siswa = mysqli_fetch_array($query)){
                                    echo "<tr class='hover:bg-purple-50/50 transition-colors duration-150 group'>";

                                    echo "<td class='py-3.5 px-4 text-center font-bold text-slate-400 group-hover:text-pastel-purple'>".$no++."</td>";
                                    echo "<td class='py-3.5 px-4 font-bold text-slate-800 group-hover:text-pastel-purple'>".htmlspecialchars($siswa['nama'])."</td>";
                                    echo "<td class='py-3.5 px-4 text-slate-600 max-w-xs truncate'>".htmlspecialchars($siswa['alamat'])."</td>";
                                    
                                    // Badge Jenis Kelamin
                                    $isPerempuan = strtolower($siswa['jenis_kelamin']) == 'perempuan';
                                    $badgeClass = $isPerempuan 
                                        ? 'bg-rose-50 text-pastel-rose border-rose-100' 
                                        : 'bg-teal-50 text-pastel-teal border-teal-100';
                                    
                                    echo "<td class='py-3.5 px-4 text-center'>";
                                    echo "<span class='inline-block px-3 py-1 text-[11px] font-bold rounded-full border shadow-sm ".$badgeClass."'>";
                                    echo ucfirst($siswa['jenis_kelamin']);
                                    echo "</span>";
                                    echo "</td>";

                                    echo "<td class='py-3.5 px-4 text-center text-slate-600 font-medium'>".htmlspecialchars($siswa['agama'])."</td>";
                                    echo "<td class='py-3.5 px-4 text-slate-600 font-medium'>".htmlspecialchars($siswa['sekolah_asal'])."</td>";

                                    // Action Buttons (Edit & Hapus)
                                    echo "<td class='py-3.5 px-4 text-center'>";
                                    echo "<div class='inline-flex items-center gap-1.5'>";
                                    echo "<a href='form-edit.php?id=".$siswa['id']."' class='px-3 py-1.5 text-xs font-bold text-pastel-purple bg-purple-50 hover:bg-pastel-purple hover:text-white rounded-xl border border-purple-100 transition-all duration-200 no-underline shadow-sm'>Edit</a>";
                                    echo "<a href='hapus.php?id=".$siswa['id']."' onclick='return confirm(\"Apakah Anda yakin ingin menghapus data pendaftaran ini?\")' class='px-3 py-1.5 text-xs font-bold text-pastel-rose bg-rose-50 hover:bg-pastel-rose hover:text-white rounded-xl border border-rose-100 transition-all duration-200 no-underline shadow-sm'>Hapus</a>";
                                    echo "</div>";
                                    echo "</td>";
                                ...
                                echo "<tr>";
                                echo "<td colspan='7' class='py-12 text-center text-slate-400 text-xs sm:text-sm font-semibold'>Belum ada data pendaftaran yang masuk.</td>";
                                echo "</tr>";
                            }
                            ?>
                ...
                <!-- Card Footer Info -->
                <div class="mt-4 pt-3 flex items-center justify-between text-xs text-slate-500 font-medium relative z-10">
                    <span class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-purple-50 border border-purple-100 font-bold text-slate-700">
                        <span class="w-2 h-2 rounded-full bg-pastel-purple animate-ping"></span>
                        Total Pendaftar: <strong class="text-pastel-purple font-extrabold"><?php echo $total; ?></strong> siswa
                    </span>
                </div>
